...
from config import PRIMARY_DATA_DIR, INTERIM_REPORTS_DIR, SECONDARY_DATA_DIR, LLM_API_KEY
...
def generate_macro_report(articles):
    ...
    """
    print(f"[{datetime.now().isoformat()}] 매크로 브리핑 보고서 생성 시작")
...
"""
    ...
    try:
        ...
        print(f"[{datetime.now().isoformat()}] 매크로 브리핑 보고서 생성 완료 (Google Search Grounding 활성화)")
        return result
...
def extract_report_keywords(client, report_markdown):
    ...
    추출된 키워드는 뉴스 기사 재필터링에 사용됩니다.
    """
    print(f"[{datetime.now().isoformat()}] 보고서 핵심 키워드 추출 시작")
...
"""
    ...
    try:
        ...
        print(f"[{datetime.now().isoformat()}] 키워드 추출 완료: {keywords}")
        return keywords
    except Exception as e:
...
def process_macro_report():
    ...
    print(f"[{datetime.now().isoformat()}] 매크로 브리핑 파일 저장 완료: {output_file}")

    # 보고서에서 핵심 키워드 추출 후 JSON으로 저장 (다음 단계 필터링용)
    ...
    print(f"[{datetime.now().isoformat()}] 키워드 파일 저장 완료: {keywords_file}")
