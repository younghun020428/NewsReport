...
from config import PRIMARY_DATA_DIR, SECONDARY_DATA_DIR, LLM_API_KEY, TARGET_SECTORS, get_now_kst
...
def load_report_keywords():
    ...
    if keywords_file.exists():
        ...
        print(f"[{now.isoformat()}] 보고서 키워드 로드 성공 ({len(keywords)}개): {keywords}")
        return keywords
    else:
        print(f"[{now.isoformat()}] 보고서 키워드 파일 없음 → 섹터 기반 필터링으로 진행 ({keywords_file})")
        return []
...
def filter_and_summarize(articles, report_keywords=None):
    ...
    """
    now = get_now_kst()
    print(f"[{now.isoformat()}] 필터링 및 요약 프로세스 시작 (총 {len(articles)}개 기사)")
    if report_keywords:
...
def process_data():
    ...
    print(f"[{now.isoformat()}] 필터링 및 요약 완료 (저장된 기사 {len(filtered_articles)}개): {output_file}")
    return output_file
